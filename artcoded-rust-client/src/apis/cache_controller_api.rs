...
/// struct for typed errors of method [`clear`]
...
/// struct for typed errors of method [`find_all9`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum FindAll9Error {
    UnknownValue(serde_json::Value),
}


pub async fn clear(configuration: &configuration::Configuration, name: &str) -> Result<models::Restore200Response, Error<ClearError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_name = name;

    let uri_str = format!("{}/api/cache/clear", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ClearError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn find_all9(configuration: &configuration::Configuration, ) -> Result<Vec<String>, Error<FindAll9Error>> {

    let uri_str = format!("{}/api/cache/find-all", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<FindAll9Error> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
