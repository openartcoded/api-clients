...
pub async fn admin_find_all(configuration: &configuration::Configuration, arg0: models::Pageable) -> Result<models::PagedModelMemZaGram, Error<AdminFindAllError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PagedModelMemZaGram`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PagedModelMemZaGram`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn delete7(configuration: &configuration::Configuration, id: &str) -> Result<(), Error<Delete7Error>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_id = id;
...
pub async fn find_all12(configuration: &configuration::Configuration, arg0: models::Pageable) -> Result<models::PagedModelMemZaGram, Error<FindAll12Error>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_arg0 = arg0;
...
pub async fn increment_views_count(configuration: &configuration::Configuration, id: &str) -> Result<(), Error<IncrementViewsCountError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_id = id;
...
pub async fn save2(configuration: &configuration::Configuration, title: &str, id: Option<&str>, description: Option<&str>, visible: Option<bool>, date_of_visibility: Option<String>, image_upload: Option<std::path::PathBuf>) -> Result<(), Error<Save2Error>> {
    // add a prefix to parameters to efficiently prevent name collisions
