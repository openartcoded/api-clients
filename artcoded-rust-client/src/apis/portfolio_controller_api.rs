...
pub async fn delete10(
    ...
    id: &str,
) -> Result<models::Restore200Response, Error<Delete10Error>> {
...
pub async fn delete_tick_from_portfolio(
    ...
    id: &str,
    symbol: &str,
) -> Result<models::Restore200Response, Error<DeleteTickFromPortfolioError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_id = id;
...
pub async fn find_all5(
    configuration: &configuration::Configuration,
) -> Result<Vec<models::Portfolio>, Error<FindAll5Error>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::Portfolio&gt;`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::Portfolio&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn find_by_id1(
    configuration: &configuration::Configuration,
    id: &str,
) -> Result<models::Portfolio, Error<FindById1Error>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_id = id;

    let uri_str = format!(
        "{}/api/finance/portfolio/find-by-id",
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Portfolio`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Portfolio`")))),
        }
    } else {
        let content = resp.text().await?;
...
) -> Result<models::Portfolio, Error<Save5Error>> {
    // add a prefix to parameters to efficiently prevent name collisions
...
pub async fn update_tag(
    configuration: &configuration::Configuration,
    id: &str,
    tick: Vec<models::Tick>,
) -> Result<(), Error<UpdateTagError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_id = id;
    let p_body_tick = tick;

    let uri_str = format!(
        "{}/api/finance/portfolio/update-ticks",
        configuration.base_path
    );
    let mut req_builder = configuration
        .client
